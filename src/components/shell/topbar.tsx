...
import { ChevronRight, Menu, RefreshCw, Search } from "lucide-react";
    ...
    <header className="sticky top-0 z-30 flex items-center justify-between gap-3 border-b border-border bg-background/80 px-3 py-3 backdrop-blur-md sm:px-6 sm:py-4">
      ...
      <div className="flex shrink-0 items-center gap-1.5 sm:gap-2">
        <button
          ...
          className="group hidden items-center gap-2 rounded-md border border-border bg-background/50 px-2.5 py-1.5 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground sm:flex"
          aria-label="打开命令面板"
        >
          <Search className="h-3.5 w-3.5" />
          <span>搜索工具…</span>
          <kbd className="rounded border border-border bg-muted px-1 py-px font-mono text-[10px] text-muted-foreground transition-colors group-hover:bg-background">
            ⌘ K
          </kbd>
        </button>
        <button
          onClick={() => setOpen(true)}
          className="grid h-9 w-9 place-items-center rounded-md border border-border bg-background text-muted-foreground transition-colors hover:bg-muted hover:text-foreground sm:hidden"
          aria-label="搜索"
        >
