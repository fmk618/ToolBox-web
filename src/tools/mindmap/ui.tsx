...
export default function MindmapUi() {
  ...
  const changeInteractionMode = (mode: MindMapInteractionMode) => {
  ...
  const addNode = (kind: "child" | "sibling" | "parent") => {
    const shouldFitAfterCreate = !selectedNode;
    const target = selectedNode ?? (kind === "child" ? snapshot.nodeData : undefined);
    if (!target) {
      setError("请先选择一个节点。");
      return;
    }
    ...
    commitGraph(nextGraph, nextData);
    setSelectedIds([nextNode.id]);
    if (shouldFitAfterCreate) {
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          void flowRef.current?.fitView({ padding: 0.24, maxZoom: 1.1 });
        });
      });
    }
  };
  ...
  return (
    <ToolShell icon={meta.icon} title={meta.name} description="自由摆放节点，保留结构关系；支持 AI、主题、导入导出" local wide>
      <div className="flex h-[calc(100dvh-13rem)] min-h-[460px] min-w-0 flex-col gap-2.5">
        ...
        <header className="relative z-40 isolate flex min-h-12 flex-wrap items-center gap-1.5 rounded-xl border border-border bg-card/90 px-3 py-2 shadow-sm backdrop-blur-md">
          ...
          <span className="ml-auto hidden text-[11px] text-muted-foreground lg:inline">{interactionMode === "select" ? "选择模式 · 双击空白或点击 + 创建" : "平移模式 · 拖动画布空白区域"}</span>
        </header>
        <div className="relative z-0 min-h-0 flex-1 overflow-hidden rounded-xl border border-border bg-card shadow-sm">
          {error && <div className="absolute left-3 top-3 z-50 max-w-md"><ErrorBox>{error}</ErrorBox></div>}
          <MindMapCanvas graph={graph} selectedIds={selectedIds} editingId={editingId} interactionMode={interactionMode} theme={snapshot.theme} {...callbacks} onNodesChange={handleNodesChange} onEdgesChange={handleEdgesChange} onNodeDragStop={handleNodeDragStop} onNodeClick={handleNodeClick} onEdgeClick={handleEdgeClick} onPaneClick={handlePaneClick} onPaneDoubleClick={handlePaneDoubleClick} onConnect={handleConnect} onMoveEnd={handleMoveEnd} defaultViewport={defaultViewport} onInit={handleInit} />
          <div className="mindmap-floating-toolbar absolute left-3 top-16 z-20 flex flex-col gap-1 rounded-xl border border-border bg-card/90 p-1.5 shadow-xl backdrop-blur-md" aria-label="画布工具">
            <Button variant={interactionMode === "select" ? "primary" : "ghost"} size="sm" onClick={() => changeInteractionMode("select")} title="选择模式：点击或拖动节点，Shift 框选" aria-label="选择工具" aria-pressed={interactionMode === "select"}><MousePointer2 className="h-4 w-4" /></Button><Button variant={interactionMode === "pan" ? "primary" : "ghost"} size="sm" onClick={() => changeInteractionMode("pan")} title="平移模式：拖动画布空白区域" aria-label="平移工具" aria-pressed={interactionMode === "pan"}><Hand className="h-4 w-4" /></Button><span className="my-0.5 h-px bg-border" /><Button variant="ghost" size="sm" onClick={() => addNode("child")} title="添加子节点（未选择时添加到根节点）" aria-label="添加子节点"><Plus className="h-4 w-4" /></Button><Button variant="ghost" size="sm" onClick={() => addNode("sibling")} disabled={!selectedNode} title="添加同级节点" aria-label="添加同级节点"><GitBranch className="h-4 w-4" /></Button><Button variant="ghost" size="sm" onClick={openInspector} disabled={!selectedNode} title="节点属性" aria-label="节点属性"><Settings2 className="h-4 w-4" /></Button><Button variant="ghost" size="sm" onClick={() => setPanel(panel === "outline" ? null : "outline")} title="大纲搜索" aria-label="大纲搜索"><Search className="h-4 w-4" /></Button>
          </div>
        ...
        <footer className="flex min-h-9 items-center justify-between rounded-xl border border-border bg-card/70 px-3 text-[11px] text-muted-foreground"><span>选择模式下双击空白或点击 + 创建 · 双击或 Enter 编辑 · 节点可自由拖动</span><span className="hidden items-center gap-2 sm:flex"><button type="button" onClick={() => void flowRef.current?.zoomOut()} aria-label="缩小"><ZoomOut className="h-3.5 w-3.5" /></button><span>画布</span><button type="button" onClick={() => void flowRef.current?.zoomIn()} aria-label="放大"><ZoomIn className="h-3.5 w-3.5" /></button></span></footer>
      </div>
    </ToolShell>
  );
}
