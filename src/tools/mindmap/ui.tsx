    ...
    <ToolShell icon={meta.icon} title={meta.name} description="自由摆放节点，保留结构关系；支持 AI、主题、导入导出" local wide>
      <div className="flex h-[calc(100dvh-13rem)] min-h-[460px] min-w-0 flex-col gap-2.5">
        ...
        <header className="relative z-40 isolate flex min-h-12 flex-wrap items-center gap-1.5 rounded-xl border border-border bg-card/90 px-3 py-2 shadow-sm backdrop-blur-md">
          ...
          <span className="ml-auto hidden text-[11px] text-muted-foreground lg:inline">{interactionMode === "select" ? "选择模式 · 双击空白创建" : "平移模式 · 拖动画布空白区域"}</span>
        </header>
        ...
        <footer className="flex min-h-9 items-center justify-between rounded-xl border border-border bg-card/70 px-3 text-[11px] text-muted-foreground"><span>选择模式下双击空白创建 · 双击或 Enter 编辑 · 节点可自由拖动</span><span className="hidden items-center gap-2 sm:flex"><button type="button" onClick={() => void flowRef.current?.zoomOut()} aria-label="缩小"><ZoomOut className="h-3.5 w-3.5" /></button><span>画布</span><button type="button" onClick={() => void flowRef.current?.zoomIn()} aria-label="放大"><ZoomIn className="h-3.5 w-3.5" /></button></span></footer>
      </div>
    </ToolShell>
  );
}
